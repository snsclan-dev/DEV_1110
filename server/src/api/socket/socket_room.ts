...
import { $SOCKET_STATE, SOCKET_LIST, SOCKET_UPDATE, SOCKET_ROOM_USER, ROOM_USER_UPDATE, SOCKET_UPDATE_TARGET, ROOM_BLOCK_CHECK, SOCKET_FIND } from "modules";
import type { SocketUser } from "types";

const socket_room = (IO: Server)=>{
    IO.on("connection", (socket) => {

        socket.on('ROOM_CREATE', async ({ name }, cb)=>{ // private chat room
            await SOCKET_UPDATE(socket, { room: socket.id, name, state: $SOCKET_STATE['8_HOST'] });
            socket.join(socket.id) // 1:1 채팅방 입장
        ...
        socket.on('ROOM_JOIN', async ({ room }, cb)=>{
            const { name } = socket.user as SocketUser;
            const $FIND = await SOCKET_FIND(room);
            if(!$FIND || $FIND.state === $SOCKET_STATE['0_WAIT']) return cb({ code: 1, msg: '대화방을 찾을 수 없습니다.' })
                socket.join(room) // 1:1 채팅방 입장
            await SOCKET_UPDATE(socket, { room, state: $SOCKET_STATE['2_CHAT'] })
            IO.to(room).emit('ROOM_MESSAGE', { status: 'JOIN', name })
            cb({ code: 0 })
        })
        socket.on('ROOM_LEAVE', async ({ type }, cb)=>{
            const { room, name } = socket.user as SocketUser;
            if(room !== socket.id) socket.leave(room);
            await SOCKET_UPDATE(socket, { room: '', state: $SOCKET_STATE['0_WAIT'] });
            IO.to(room).emit(`ROOM_MESSAGE`, { status: type === 'chat' ? 'END' : 'LEAVE', name });
            await SOCKET_ROOM_USER(IO, socket)
            cb({ code: 0, msg: '대화를 종료합니다.' });
        })
        ...
        //     await ROOM_USER_UPDATE(IO, socket)
        // })
        socket.on('ROOM_BLOCK', async ({ room, targetSocketId, target_name })=>{ // 차단
            await SOCKET_UPDATE_TARGET(targetSocketId, { num: '', room: '', state: $SOCKET_STATE['0_WAIT'] })
            socket.to(targetSocketId).emit('ROOM_BLOCK')
            IO.in(targetSocketId).socketsLeave(room);
