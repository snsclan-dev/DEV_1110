...
export const $REGEX_GUIDE = {
    ...
    // room
    room_code: '방 코드를 입력해 주세요. (20)', room_title: '방 이름을 입력해 주세요. (4~20)', room_pass: '방 입장 비밀번호를 입력해 주세요. 한/영, 숫자 (4~10)', room_max: '방 참여자를 입력해 주세요 (2~99)',
    // board
...
const checkLength = (input: string, min: number, max: number, label?: string): CodeData => {
    if(!input && min > 0) return { code: 1, msg: `${label} : 내용을 입력해 주세요.` }
    if(min === max && input.length !== min) return { code: 1, msg: `${label} : ${min}자가 아닙니다. (${min})`}
    if (min !== 0 && input.length < min) return { code: 1, msg: `${label} : ${min}자 이상 입력해 주세요. (${min}~${max})` };
    if (input && input.length > max) return { code: 1, msg: `${label} : ${max}자 이하로 입력해 주세요. (${min}~${max})` }
    return { code: 0 };
};
const checkRule = (key: string, value: number | string | boolean, admin = false): CodeData => {
    const $RULE = $REGEX_RULES[key];
    if (!$RULE) return { code: 0 };
    if(typeof value === 'string'){
        const invalidInput = value.match($REGEX_INPUT) || [];
        if (invalidInput.length > 0) return { code: 1, msg: `${$RULE.label} : 허용되지 않는 문자가 포함되어 있습니다.`, note: `허용되지 않는 문자 : ${invalidInput.join(", ")}` };
        if (!admin && $RULE.invalid) {
            const invalid = value.match($RULE.invalid) || [];
            if (invalid.length > 0) return { code: 1, msg: `${$RULE.label} : 허용되지 않는 문자가 포함되어 있습니다.`, note: `허용되지 않는 문자 : ${invalid.join(", ")}` };
        }
        if ($RULE.length) {
    ...
    if (($RULE.check && !$RULE.check(value)) || (typeof value === 'string' && $RULE.regex && !$RULE.regex.test(value))){
        return { code: 1, msg: $RULE.label ? `${$RULE.label} : 양식에 맞게 입력해 주세요.` : $RULE.msg, note: $REGEX_GUIDE[key as keyof typeof $REGEX_GUIDE] ?? null };
    }
    return { code: 0 };
};
// ========== client <=> server
...
export const checkInput = (inputs: Record<string, string | null | undefined>, admin = false): CodeData => {
    const sanitizedInputs: Record<string, string | null> = {};
    for (const [key, value] of Object.entries(inputs)) {
        if (value !== undefined && value !== null) {
            const result = checkRule(key, value, admin);
...
type Check = { folder: string, image: string, refImage?: string | null }
export const checkImage = async ({ folder, image, refImage = null }: Check): Promise<CodeData> =>{ // input image(thumbnail)
    if(image === refImage) return { code: 0, data: image }
    let $URL: string | null = null;
    try{
        if(image){
            const $FOLDER_TEMP = image.replace($REGEX.input_image, '$1');
            ...
            const $FILE_NAME = image.replace($REGEX.input_image, '$2'); // file name
            try {
        ...
        if (refImage && (!image || image !== refImage)) { // 동기
            fs.access(refImage).then(() => fs.unlink(refImage)).catch(err => checkError(err, '/modules/REGEX.js, checkImage > unlink'));
        }
