...
export const reqUserToken = (req: Request, res: Response, next: NextFunction) => {
    ...
    console.log('reqUserToken :', check);
    if (!check || check === 'undefined' || !req.user || !req.user.id) {
        req.user = { id: null, level: 0, ip: ip } as User;
        return next();
    }
    // jwt.verify(check, JWT_KEY as string, (err: jwt.VerifyErrors | null, decoded: string | jwt.JwtPayload | undefined) => {
    ...
    //     next();
    // });
    jwt.verify(check, JWT_KEY as string, (err: VerifyErrors | null, decoded: string | JwtPayload | undefined) => {
        if (err) {
...
export const errorHandler = (err: any, req: Request, res: Response, next: NextFunction) => {
    const note = `Request URL: ${req.originalUrl}`;
