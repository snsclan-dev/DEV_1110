...
import { storeApp, storeUser, useInput, useModal, $REGEX } from "modules"
import { Modal, Modal_Image, Modal_Video, View_Char } from 'components/app';
import { $CHAT_MESSAGE } from "components/room"
import { Chat_Preview_Image, Chat_Preview_Video, Chat_Upload } from "components/chat"
...
export const Room_Chat = ({ room, view, monitor }: { room: Room, view: Chat[], monitor: string })=>{
    ...
    const onPaste = (e: ClipboardEvent)=>{
        ...
        const $IMAGE: string[] = [...new Set($TEXT_IMAGE.concat($HTML_REPLACE_IMAGE))];
        const $VIDEO: string[] = [...new Set($TEXT_VIDEO.concat($HTML_REPLACE_VIDEO))];
    ...
    // const CHAT_BOX = ({ obj, line }: { obj: Chat, line: number }) => {
    //     const $NAME = $ADMIN ? view[line - 1]?.name !== view[line]?.name : view[line - 1]?.name !== view[line]?.name && obj.name !== user.name
    //     const $USER = $ADMIN ? room.user_id === obj.id : obj.name === user.name
    //     if (obj.notice) return null;
    //     return (<div className={$USER ? "chat_box_right" : "chat_box_left"}>
    ...
    const CHAT_MENU = ()=>{
        return(<>
            ...
            <button className='bt_w100 c_green' onClick={()=>openModal({ link_image: true })}>이미지 주소(URL) 전송</button>
            <button className='bt_w100 c_green' onClick={()=>openModal({ link_video: true })}>동영상 주소(URL) 전송</button>
            <button className='bt_w100 c_blue' onClick={()=>openModal({ upload: true })}>이미지 파일 전송</button>
        </>)
    }
    ...
    return(<div className="layout_max">
        ...
        {modal.paste_video && <Modal title='동영상 미리보기' onClose={closeModal}><Chat_Paste_Video closeModal={closeModal} data={modal.video}/></Modal>}
        ...
        {monitor !== 'ADMIN' && <div className="align">
            ...
            <button className="bt_chat_message c_orange" onClick={()=>openModal({ menu: true })}>등록</button>
        </div>}
    </div>)
}