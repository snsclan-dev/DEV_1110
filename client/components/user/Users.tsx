...
import { checkAdmin, checkManager } from "modules/systems";
...
//     if ($FIND) return <span className={`${$FIND.style} mg_r6`}>{$FIND.level_name ?? level}</span>;
//     return <span className="level_lgray">X</span>;
// };
export const User_Level = ({ level }: { level: number }) => {
    const $FIND = $USER_LEVEL.find((e) => e.level_min <= level && e.level_max >= level);
    if ($FIND) return <button className={`${$FIND.style} mg_r6`}>{$FIND.level_name ?? level}</button>;
    return <button className="level_lgray">X</button>;
};
...
export const userState = (obj: { blocked: string, state: number })=>{
    ...
    if(blocked && blocked >= getDate()) return $USER_STATE['7_block'];
    return state;
}
...
export const User_Id = ({ obj, user }: { obj: User, user: User }) => {
    if (!user.id || !obj.id) return null;
    const { id, level } = obj;
...
export const User_Ip = ({ obj, user }: { obj: User, user: User }) => {
    const { ip, level } = obj;
    ...
    const $IP = /(\d+)[.](\d+)[.](\d+)[.](\d+)/g;
    return ip.replace($IP, "$1.$2.🔒.🔒")
    // return (<span className="fs_13 c_blue">{ip.replace($IP, "$1.$2.🔒.🔒")}</span>)
