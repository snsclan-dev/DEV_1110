...
export const Title = ({ title, color = 'c_gray', children }: { title: string, color?: string, children?: ReactNode })=>{
    return(<div className="box_lgray flex_align pd_6 mg_b20">
...
// export const Title = ({ color='c_gray', children }: Tag)=>{
//     return(<p className={`fwb pd_h10 ta_c ${color}`}>{children}</p>)
// }
...
export const Li = ({ color = 'blue', style = '', children }: Tag) => {
    return (<div className={`li_grid ${style}`}>
        <span><Svg name="li" size={10} color={color} /></span>
        <span>{children}</span>
    </div>);
}
export const Li_Item = ({ color = 'blue', style = '', children }: Tag)=>{
    return (<div className={`li_grid ${style}`}>
        <span className="pd_l3"><Svg name={name} size={12} color={color} /></span>
        <span>{children}</span>
    </div>);
}
