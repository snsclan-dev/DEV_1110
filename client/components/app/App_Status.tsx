...
import { $REGEX_GUIDE, checkInput, socket, socketData, storeApp, storeChat, storeRoom, storeUser, useFetch, useInput, useInputColor } from "modules"
...
export const App_Status = ({ router, status }: { router: string, status: number })=>{
    const { setPopup } = storeApp((s)=> s)
    const { user, setUser } = storeUser((s)=> s)
    const { chat, setChat } = storeChat((s)=>s)
    const { room, setRoom } = storeRoom((s)=>s)
    ...
    const clickCheckName = ()=>{
        ...
        socket.emit('USER_NAME', { name: input.name }, (data: CodeData)=>{
            ...
            if($DATA){
                ...
                setChat({ name: input.name, status: $APP_STATUS['2_WAIT'] })
            }
        })
    }
    const clickCheckRoom = async ()=>{
        ...
        if($DATA){
            ...
            setRoom({ room: input.room_code, status: $APP_STATUS['2_WAIT'] })
        }
    }
    const clickDelete = ()=>{
        setChat({ name: null, status: 0 })
        setInput({ name: '' })
        Cookies.remove('name')
        Cookies.remove('room')
        setPopup({ msg: '모든 입력 정보가 삭제되었습니다.' })
    }
    ...
    return (<div className="layout_max">
        ...
        <div className="max_w40 mg_5a">
            {router === 'CHAT' && status < $APP_STATUS['2_WAIT'] && <>
            ...
            {router === 'ROOM' && status < $APP_STATUS['2_WAIT'] && <>
        ...
        <div className="pd_h6 ta_c">
            ...
            <Button style="bt_main c_red" onClick={clickDelete}>입력 정보 삭제</Button>
        </div>
    </div>);
}