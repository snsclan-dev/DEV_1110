...
import { Chat_Paste_Media, Chat_Preview_Media, Chat_Upload, chatMessage, chatSlider } from "components/chat"
...
const CHAT_BOX = memo(({ monitor, room, obj, line, user, message, openModal }: 
    ...
    const nameStyle = (e: ChatMessage)=>{
        if(checkAdmin(e.level)) return 'c_orange'
        if(e.id === room.user_id) return 'c_blue'
        return 'c_black'
    }

    return (<div className={$USER ? "chat_box_right" : "chat_box_left"}>
        {$NAME && <p className={$USER ? 'pd_r ta_r' : 'ta_l'}>
            {/* {checkAdmin(obj.level) ? '☀️ ' : room.user_id === obj.id && '⭐ '}<span className={`${$USER ? 'c_orange' : 'c_blue'} pd_l fs_13 fwb`}>{obj.name}</span> */}
            {/* {checkAdmin(obj.level) ? <span className='emg_r'>☀️</span> : room.user_id === obj.id && <span className='emg_r'>⭐</span>}<span className={`${$USER ? 'c_orange' : 'c_blue'} pd_l fs_13 fwb`}>{obj.name}</span> */}
            {checkAdmin(obj.level) ? <span className='emg_r'>☀️</span> : room.user_id === obj.id && <span className='emg_r'>⭐</span>}<span className={`${nameStyle(obj)} pd_l fs_13 fwb`}>{obj.name}</span>
        </p>}
...
export const Chat_View = ({ monitor = false }: { monitor?: boolean })=>{
    ...
    const CHAT_NOTICE = ({ message }: { message: ChatMessage }) => {
        ...
        <div className="layout_view">
            <div id="view" ref={refView} className="chat_view" onPaste={onPaste}>
                {message.map((e, i)=> <Fragment key={i}>
                    {/* <CHAT_NOTICE status={e.status} me id={e.id} name={e.name} level={e.level}/> */}
                    <CHAT_NOTICE message={e}/>
