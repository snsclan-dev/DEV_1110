import { Kakao_Map, socket, socketData, storeApp, storeChat, storeUser, useModal } from "modules";
import { checkAdmin } from "modules/systems";
import { $APP_STATUS, Char, Label, Modal, Svg, View_Distance } from "components/app";
import { User_Ip } from "components/user";
import { $SOCKET_STATE } from "components/chat";
import type { CodeData, User } from "types";

type ChatHeader = 'CHAT' | 'ROOM'
export const Chat_Header = ({ monitor = false, type = 'CHAT' }: { monitor?: boolean, type?: ChatHeader })=>{ // type: chat, room, private ...
    const { setStatus, setPopup, setConfirm } = storeApp((s)=>s)
    const { user } = storeUser((s)=>s)
    const { chat, room, setRoom, roomUser, clear } = storeChat((s)=> s)
    const { modal, openModal, closeModal } = useModal()

    const $USER = roomUser.sort((a, b) => { // 참여자 정렬
        if (a.name === chat.name) return -1;
        if (b.name === chat.name) return 1;
    ...
    const userStyle = (e: User)=>{
        ...
        if(e.name === chat.name) return { border: 'room_user_host', color: 'c_blue' }
    ...
    const clickCopy = ()=>{
        ...
        setPopup({ msg: "대화방 코드와 주소가 복사되었습니다.\n붙여넣기(Ctrl + V) 하세요." });
        return closeModal()
    }
    const clickStatus = (status: string)=>{
        if(status === 'ADMIN_LEAVE'){
            socket.emit('ADMIN_LEAVE', (data: CodeData)=>{
                ...
                if($DATA) setStatus($APP_STATUS['2_WAIT']); clear();
            })
        }
        if(status === 'LEAVE'){
            const $CONFIROM = ()=>{
                socket.emit('ROOM_LEAVE', { type }, ( data: CodeData)=>{
                    const $DATA = socketData(data)
                    if($DATA){
                        setRoom({ room: '' })
            ...
            setConfirm({ msg: '대화를 종료하시겠습니까?', confirm: $CONFIROM})
        }
    }
    const Chat_Menu = ()=>{
        return(<div className='wrap_flex_bt'>
            {room.room === socket.id && <button className="flex_bt c_green" onClick={clickCopy}>대화방 주소 복사</button>}
            ...
            {monitor ? <button className="flex_bt c_red" onClick={()=>{ clickStatus('ADMIN_LEAVE'); closeModal() }}>모니터링 종료</button> : 
                <button className="flex_bt c_red" onClick={()=>{ clickStatus('LEAVE'); closeModal() }}>{type === 'CHAT' ? '대화 종료' : '대화방 나가기'}</button>}
        </div>)
    }
    const Chat_Profile = ({ obj }: { obj: User })=>{
        return(<>
            <p className="input mg_b10">
                <Label label="대화명"><span className="fwb">{obj.name}</span>{obj.state === 1 && <span className="c_blue fwb"> (⭐방장)</span>}</Label>
                {obj.name === chat.name && <span className="room_user_me mg_l6">나</span>}
            </p>
            <p className="input mg_b10"><Label label="아이피"><User_Ip obj={obj} user={user}/></Label></p>
            {obj.name !== chat.name && <p className="input mg_b10"><Label label="나와의 거리"><View_Distance type="text" location1={user.location!} location2={obj.location!}/></Label></p>}
            {type === 'ROOM' && <div className="ta_c pd_t3">
        ...
        <div className="layout_max pd_6">
            <div className="box pd_10 mg_b10">
                ...
                {$USER.map((e)=> <div key={e.name} className={`${userStyle(e).border} fs_13 fwb`} onClick={()=>openModal({ profile: true, data: e })}>
                    ...
                    {e.name !== chat.name ? <span className="room_user_me mg_l6">나</span> : <><Char char="vl"/><View_Distance location1={user.location!} location2={e.location!}/></>}
                </div>)}
            </div>
        </div>
    </>)
