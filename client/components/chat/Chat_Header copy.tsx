...
import { $SOCKET_STATE } from "./Chat_Utils";

export const Chat_Header = ({ monitor = false, clickStatus }: { monitor?: boolean, clickStatus: (data: string)=> void })=>{
    ...
    const clickBlock = (obj: User)=>{
        const $CONFIRM = async ()=>{
            const $DATA = await useFetch.post('/room/block', { room: room.room, target_socket: obj.socket })
            if($DATA){
                socket.emit('ROOM_BLOCK', { room: room.room, target_socket: obj.socket, target_name: obj.name })
                closeModal()
            }
        }
    ...
    const Chat_Menu = ()=>{
        return(<div className='wrap_flex_bt'>
            {(room.user_id === user.id || room.room === socket.id) && <>
                <button className="flex_bt c_green" onClick={clickCopy}>대화방 주소 공유</button>
                <button className="flex_bt c_orange" onClick={()=>openModal({ modify: true })}>대화방 설정</button>
            </>}
            {/* <button className="flex_bt c_green" onClick={clickCopy}>방 이름(코드) 복사하기</button> */}
        ...
        <div className="layout_max pd_6">
            <div className="box pd_10 mg_b10">
                ...
                {$USER.map((e)=> <div key={e.name} className={`${userStyle(e).border} fs_13 fwb`} onClick={()=>openModal({ profile: true, data: e })}>
                    {e.state === $SOCKET_STATE['8_HOST'] && <span className="emg_r">⭐</span>}<span className={userStyle(e).color}>{e.name}</span>
                    {e.name === user.name ? <span className="room_user_me mg_l6">나</span> : <><Char char="vl_1"/><span className="emg_l"><View_Distance location1={user.location!} location2={e.location!}/></span></>}
                </div>)}
            </div>
        </div>
    </>)
