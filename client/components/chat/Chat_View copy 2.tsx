...
const Chat_Message = memo(({ monitor, room, obj, line, user, message, openModal }: 
    ...
    return (<div className={$USER ? "chat_box_right" : "chat_box_left"}>
        {$NAME && <p className={`fs_13 fwb ${$USER ? 'ta_r' : 'ta_l'}`}>{room.user_id === obj.id && <span className='emg_r'>⭐</span>}<span className={obj.id === room.user_id ? 'c_blue' : 'c_black'}>{obj.name}</span></p>}
        ...
        <div className="layout_chat_view">
            <div id="view" ref={refView} className="chat_view" onPaste={onPaste}>
                {message.map((e, i)=> <Fragment key={i}>
                    ...
                    <Chat_Message monitor={monitor} room={room} obj={e} line={i} user={user} message={message} openModal={openModal}/>
                </Fragment>)}
            </div>
