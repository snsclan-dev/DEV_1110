...
import { storeApp, useInput, useModal, $REGEX, storeChat, socket } from "modules"
import { $FILE_UPLOAD } from 'modules/systems';
import { Li, Modal, Modal_Image, Modal_Video } from 'components/app';
import { $CHAT_MESSAGE, Chat_Paste_Media, Chat_Preview_Media, Chat_Upload, chatMessage, chatSlider } from "components/chat"
import type { RoomMessage, ChatMessage, RoomData, ChatName } from "types"

const Chat_Message = memo(({ monitor, room, obj, line, chatName, view, openModal }: 
    { monitor?: boolean, room: RoomData, obj: ChatMessage, line: number, chatName: ChatName, view: ChatMessage[], openModal: (modal: Record<string, any>)=> void }) => {
    if (obj.status) return null;
    const prevName = view[line - 1].status || view[line - 1]?.name;
    const currName = view[line]?.name;
    const $NAME = monitor ? prevName !== currName : prevName !== currName && obj.name !== chatName
    const $USER = monitor ? room.user_id === obj.id : obj.name === chatName
    ...
    return (<div className={$USER ? "chat_box_right" : "chat_box_left"}>
        {$NAME && <p className={`fs_13 fwb ${$USER ? 'ta_r' : 'ta_l'}`}>{room.user_id === obj.id && <span className='mg_r2'>⭐</span>}<span className={obj.id === room.user_id ? 'c_blue' : 'c_black'}>{obj.name}</span></p>}
        ...
        {($IMAGE || $VIDEO) && (<>
            <div className='layout_media'>
                {obj.image?.map((src, i) => (<img key={i} className='chat_media' src={src} onClick={() => openModal({ image: src })} />))}
                {obj.video?.map((src, i) => (<video key={i} className='chat_media' src={src} onClick={() => openModal({ video: src })} />))}
            </div>
            <p className='fs_13 c_green fwb ta_c mg_t6'>{obj.image ? '이미지' : '동영상'}</p>
        </>)}
    </div>);
}, (prev, next)=>{
    return prev.obj === next.obj && prev.line === next.line; // 불변 객체 기준 최적화
});
Chat_Message.displayName = "Chat_Message";

export const Chat_View = ({ monitor = false }: { monitor?: boolean })=>{
    const refInput = useRef<HTMLInputElement>(null)
    const refView = useRef<HTMLDivElement>(null)
    const { setPopup } = storeApp((s)=>s)
    const { chat, room, message } = storeChat((s)=> s)
    ...
    const onPaste = (e: ClipboardEvent)=>{
        e.preventDefault();
        const screen = e.clipboardData.items; // 스크린(클립보드) 캡쳐
        const $SCREEN = Array.from(screen).find(e=> e.type.indexOf('image') !== -1)
        if($SCREEN){
            const blob = $SCREEN.getAsFile();
                if (blob) {
                    if(blob.size > $FILE_UPLOAD.fileSize) return setPopup({ msg: "이미지 용량이 너무 큽니다.", note: '최대 10MB 까지 전송이 가능합니다.' })
                    const reader = new FileReader();
                    reader.onload = () => {
                        const base64Image = reader.result as string;
                        openModal({ paste_image: true, data: { image: [base64Image], text: "" } });
                    };
                    reader.readAsDataURL(blob);
                }
            return;
        }
        const $LENGTH = 5; // 붙여넣기 갯수
        ...
        const $IMAGE: string[] = [...new Set($TEXT_IMAGE.concat($HTML_REPLACE_IMAGE))].filter((e): e is string => !!e).slice(0, $LENGTH);
        const $VIDEO: string[] = [...new Set($TEXT_VIDEO.concat($HTML_REPLACE_VIDEO))].filter((e): e is string => !!e).slice(0, $LENGTH);
        
        if($IMAGE.length){
            if($IMAGE.length > $LENGTH) return setPopup({msg: <>
                <p>최대 <span className="c_blue fwb">{$LENGTH}개</span>의 이미지만 붙여넣기가 가능합니다.</p>
        ...
        if($VIDEO.length){
            if($VIDEO.length > $LENGTH) return setPopup({msg: <>
                <p>최대 <span className="c_blue fwb">{$LENGTH}개</span>의 동영상만 붙여넣기가 가능합니다.</p>
    ...
    const Room_Message = ({ message }: { message: ChatMessage }) => {
        // const { status, id, name, level } = message
        const { status, name } = message
        if(!status) return null;
        const $STATUS: Partial<Record<RoomMessage, { color: string, text: string }>> = {
            ADMIN: { color: 'chat_notice_orange', text: '모니터링을 시작합니다.' },
            CREATE: { color: 'chat_notice_blue', text: '대화방을 만들었습니다.' },
            JOIN: { color: 'chat_notice_blue', text: `[ ${name} ]님이 입장하였습니다.` },
            LEAVE: { color: 'chat_notice_gray', text: `[ ${name} ]님이 퇴장하였습니다.` },
            BLOCK: { color: 'chat_notice_red', text: `[ ${name} ]님을 차단하였습니다. (내보내기)` },
            DELETE: { color: 'chat_notice_red', text: `대화방이 삭제되었습니다.` },
            END: { color: 'chat_notice_red', text: `대화방이 종료되었습니다.` },
        }
        const $NOTICE = $STATUS[status];
        if (!$NOTICE) return null;
        // return(<div className={$NOTICE.color}>{checkAdmin(level) ? '☀️ ' : room.user_id === id && '⭐ '}{$NOTICE.text}</div>);
        return(<div className={$NOTICE.color}>{$NOTICE.text}</div>);
    };
    const CHAT_MENU = ()=>{
        return(<>
            <Li>이미지(동영상)의 주소(URL) 전송이 가능합니다. <span className="c_blue fwb">(복사 &gt; 붙여넣기)</span></Li>
            <Li><span className="c_blue fwb">복사</span>한 주소(URL)를 메세지 입력칸에 <span className="c_blue fwb">붙여넣기</span> 하세요.</Li>
            <div className='wrap_flex_bt mg_t10'>
    ...
    return(<div className="layout_max">
        ...
        <div className="layout_chat_view">
            <div id="view" ref={refView} className="chat_view" onPaste={onPaste}>
                {message.map((e, i)=> <Fragment key={i}>
                    <Room_Message message={e}/>
                    <Chat_Message monitor={monitor} room={room} obj={e} line={i} chatName={chat.name} view={message} openModal={openModal}/>
                </Fragment>)}
            </div>
            <div id="slider" className="chat_view_slider"></div>
        </div>

        {!monitor && <div className="layout_chat_input">
