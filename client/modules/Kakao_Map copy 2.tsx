...
export const Kakao_Map: React.FC<{ user: User[] }> = ({ user }) => {
    ...
    useEffect(() => {
        ...
        groups.forEach(({ location, users }) => {
            ...
            // 말풍선 꼬리 포함 HTML 콘텐츠
            // const content = `<div class="speech-bubble">${users.map(u => u.name).join(', ')}</div>`;
            const content = `
                <div style="
                    position: relative;
            ...
            // const overlay = new window.kakao.maps.CustomOverlay({ position, content, yAnchor: 1, map: mapRef.current }); // yAnchor: 오버레이가 마커 위쪽에 위치하게 조정 (1은 하단 기준)
            const overlay = new window.kakao.maps.CustomOverlay({ position, content, yAnchor: 1, map: mapRef.current }); // yAnchor: 오버레이가 마커 위쪽에 위치하게 조정 (1은 하단 기준)
