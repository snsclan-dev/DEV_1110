import Cookies from "js-cookie";
import { storeApp } from "./stores/zustand";

// client fetch
const createFetch = async (method: 'get' | 'post', url: string, data?: any): Promise<any> => {
    const $URL = method === 'get' && data ? `${url}?${new URLSearchParams(data).toString()}` : url;
    const baseUrl = `${process.env.NEXT_PUBLIC_SERVER_API}${$URL}`;
    const { setLoading, setPopup } = storeApp.getState();
    setLoading(true)
    const $FORM = data instanceof FormData;
    const headers: HeadersInit = { Authorization: `Bearer ${Cookies.get(process.env.NEXT_PUBLIC_APP_NAME!)}`, ...($FORM ? {} : { "Content-Type": "application/json" }) }
    try{
        const $FETCH = await fetch(baseUrl, {
            method: method.toUpperCase(),
            headers,
            credentials: 'include',
            cache: 'no-cache', // 서버 캐시
            // body: method === 'post' ? JSON.stringify(data) : undefined,
            body: $FORM ? data : method === 'post' ? JSON.stringify(data) : undefined,
        })
        const $DATA = await $FETCH.json()
        if(process.env.NODE_ENV === 'development') console.log('FETCH DATA :', $DATA); ///
        if(!$FETCH.ok) return setPopup({ code: 2, msg: '서버가 응답하지 않습니다.', note: '오류(코드): CLIENT_CREATE_FETCH' })
        const { code, msg } = $DATA;
        if(code === 9){
            Cookies.remove(process.env.NEXT_PUBLIC_APP_NAME!)
            setPopup({ code: 9, msg: '로그인이 필요합니다', note: '로그인 시간 만료.' })
        }
        if(msg) setPopup($DATA)
        if(code > 0) return null;
        return $DATA;
    }catch(err){
        setPopup({ code: 2, msg: '서버가 응답하지 않습니다.', note: `오류(코드): CLIENT_CREATE_FETCH\n${err}` })
    }finally{
        setLoading(600)
    }
}
export const useFetch = {
    get: (url: string, data?: any): Promise<any> => createFetch('get', url, data),
    post: (url: string, data?: any): Promise<any> => createFetch('post', url, data )
}